...
from network.arch_w_normal import PIFUHrnet
import config
from util import lr_schedule
import util.utils as util
import util.obj_io as obj_io
from dataloader.dataloader_rgb import ImgLoader
# import spconv
...
class Trainer(object):
    ...
    def train_network(self, dataset_dir, output_dir, epoch_num,
        ...
        for epoch in range(start_epoch, epoch_num):
            ...
            for i, items in enumerate(loader):
                ...
                depth_normal_batch, mask_batch, pts_batch = (
                ...
                _, loss, logs = self.forward(
                    depth_normal_batch, mask_batch, pts_batch, gt_batch,
    ...
    def eval_network(self, dataset_dir, model_dir, output_dir):
        ...
        for i in range(len(loader.dataset.data_list)):
            ...
            if (view % 1 == 0):
                ...
                depth_normal_batch = items['depth_normal'].unsqueeze(0)
                ...
                outputs, _, _ = self.forward(
                    depth_normal_batch, mask_batch, pts_batch,  None,
                ...
                # release memory
                del items, pts_batch, outputs, vertices, pts_ov, depth_normal_batch, mask_batch,
                # except:
    ...
    def forward(self, rgbd_imgs_batch, masks, pts, gt, training=True, pc_smpl_semantic=None, pc_joints=None):
        # print(rgbd_imgs_batch.dtype,masks.dtype,masks.dtype,pts.dtype,pc_joints.dtype,gt.dtype)
        outputs = self.pifu(rgbd_imgs_batch, masks, pts,pc_smpl_semantic, pc_joints)
        if training:
            recon_loss = 0
