...
if __name__ == '__main__':
    ...
    smpl = TetraSMPL('/home/lpp/Projects/PaMIR-main/networks/data/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl',
                         '/home/lpp/Projects/PaMIR-main/networks/data/tetra_smpl.npz')
    # pose = np.random.randn(1, 72) * 0.2
    # shape = np.random.randn(1, 10) * 0.3
    # pose = torch.from_numpy(pose).float()
    # shape = torch.from_numpy(shape).float()

    pkl_file = '/media/lpp/KESU/twindom_with_smpl/126111535847796-h/smpl_recali_param.pkl'
    #
    smpl_pkl = pkl.load(open(pkl_file, 'rb'), encoding='latin-1')
    # print(smpl_pkl.keys())
    pred_shape = smpl_pkl['betas']
    body_pose = smpl_pkl['body_pose']
    global_orient = smpl_pkl['global_orient']
    global_transl = smpl_pkl['global_body_translation']
    body_scale = smpl_pkl['body_scale']
    # print(body_scale)
    shape, pose, global_transl, global_orient = torch.tensor(pred_shape), torch.tensor(body_pose), \
                                                          torch.tensor(global_transl).unsqueeze(0), torch.tensor(
        global_orient)
